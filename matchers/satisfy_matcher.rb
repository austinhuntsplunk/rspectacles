...
RSpec.describe 'Custom Conditions' do
    describe 'new vehicle' do
        let(:vehicle) { Vehicle.new(color: 'blue', mileage: 0, make: 'Make', model: 'Model', year: Time.now.year - 1, safe: true) }
        ...
        it 'is safe' do
            ...
            # or using a block
            expect(vehicle).to satisfy('safe vehicle') do |v|
                v.safe?
            end
        end
    end
    ...
    describe 'old vehicle' do
        let(:vehicle) { Vehicle.new(color: 'red', mileage: 100000, make: 'OldMake', model: 'OldModel', year: 1990, safe: false) }
        ...
        it 'has mileage more than 50000' do
            expect(vehicle).to satisfy { |v| v.mileage > 50000 }
            # or 
