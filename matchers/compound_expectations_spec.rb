...
RSpec.describe 'Compound Expectations' do
    ...
    describe 25 do 
        it 'is an odd perfect square with satisfy' do 
            expect(subject).to satisfy { |n| n.odd? && Math.sqrt(n) % 1 == 0 }
        end
        it 'is an odd perfect square with and' do 
            expect(subject).to be_odd.and satisfy { |n| Math.sqrt(n) % 1 == 0 }
        end
        ...
        it 'is a multiple of 5 and not a multiple of 3' do
            expect(subject).to satisfy { |n| n % 5 == 0 && n % 3 != 0 }
        end

        it {is_expected.to be_odd.and satisfy { |n| Math.sqrt(n) % 1 == 0 }}
    end
