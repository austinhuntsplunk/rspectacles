...
RSpec.describe Movie do
  let(:stuntman) do
    double('Mr. Danger', name: 'Mr. Danger', specialty: 'Stunts', age: 30, gender: 'Male', ready: true)
  end
  let(:romantic_lead) do
    double('Ms. Heartthrob', name: 'Ms. Heartthrob', specialty: 'Romance', age: 28, gender: 'Female', ready: true)
  end

  # create a movie subject with the mocked actor
  subject do
    described_class.new(title: 'Action-Packed Romance', director: 'Jane Smith', release_year: 2023,
                        genre: 'Action/Romance', cast: [stuntman, romantic_lead])
  end
  ...
  describe '#shoot_scene' do
    ...
    it 'does not shoot a scene if not all actors are ready' do
      ...
      expect do
        subject.shoot_scene('The Big Chase')
      end.to output(/Cannot shoot scene 'The Big Chase' because not all actors are ready/).to_stdout
